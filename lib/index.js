...
const addSeconds_1 = __importDefault(require("date-fns/addSeconds"));
...
function getDayFraction(date) {
    return (getMinutes_1.default(date) / 60 + getHours_1.default(date)) / 24;
}
function getDecimalNumbers(value, defaultNumberOfDecimals = 4) {
    return Number(value.toFixed(defaultNumberOfDecimals));
}
// Allow "mocking" the tzOffset.
function getJulianDate(date, tzOffset = date.getTimezoneOffset()) {
    const decimal = getDayFraction(date);
    const tzSubtractor = (-1 * tzOffset) / 1440;
    return (differenceInCalendarDays_1.default(date, new Date(1900, 0, 1, 12)) +
...
function getBaselineNumbers(date, tzOffset) {
    ...
    const obliqCorr = meanObliqEcliptic + 0.00256 * cos(radians(125.04 - 1934.136 * jc));
    const vary = pow(tan(radians(obliqCorr / 2)), 2);
    // Sun equation of thecenter.
    const sunEqCtr = sin(radians(gmAnomSun)) * (1.914602 - jc * (0.004817 + 0.000014 * jc)) +
    ...
    const sunAppearLong = sunTrueLongitude - 0.00569 - 0.00478 * sin(radians(125.04 - 1934.136 * jc));
    // Sun declination.
    const sunDecl = degrees(asin(sin(radians(obliqCorr)) * sin(radians(sunAppearLong))));
    // Equation of time.
    const eq = 4 *
        degrees(vary * sin(2 * radians(gmLongSun)) -
...
function getBaselineNumbersForSunriseAndSunset({ date, latitude, longitude, tzOffset = date.getTimezoneOffset() }) {
    const { eq, sunDecl } = getBaselineNumbers(date, tzOffset);
    const haSunrise = degrees(acos(cos(radians(90.833)) / (cos(radians(latitude)) * cos(radians(sunDecl))) -
...
function getSolarNoonLSTInSeconds(params) {
    const { solarNoon } = getBaselineNumbersForSunriseAndSunset(params);
    return solarNoon * 86400;
}
exports.getSolarNoonLSTInSeconds = getSolarNoonLSTInSeconds;
function getSunriseInSeconds(params) {
    const { haSunrise, solarNoon } = getBaselineNumbersForSunriseAndSunset(params);
    const solarNoonInSeconds = solarNoon * 86400;
    return solarNoonInSeconds - ((haSunrise * 4) / 1440) * 86400;
}
exports.getSunriseInSeconds = getSunriseInSeconds;
function getSunsetInSeconds(params) {
    const { haSunrise, solarNoon } = getBaselineNumbersForSunriseAndSunset(params);
    const solarNoonInSeconds = solarNoon * 86400;
    return solarNoonInSeconds + ((haSunrise * 4) / 1440) * 86400;
}
exports.getSunsetInSeconds = getSunsetInSeconds;
// Functions that return the fraction.
function getSolarNoonLSTInFractions(params) {
    return getSolarNoonLSTInSeconds(params) / 86400;
}
exports.getSolarNoonLSTInFractions = getSolarNoonLSTInFractions;
function getSunriseInFractions(params) {
    return getSunriseInSeconds(params) / 86400;
}
exports.getSunriseInFractions = getSunriseInFractions;
function getSunsetInFractions(params) {
    return getSunsetInSeconds(params) / 86400;
}
exports.getSunsetInFractions = getSunsetInFractions;
// Functions to get the exact date.
function getSolarNoonLSTDate(params) {
    const { solarNoon } = getBaselineNumbersForSunriseAndSunset(params);
    const solarNoonInSeconds = solarNoon * 86400;
    const start = startOfDay_1.default(params.date);
    return addSeconds_1.default(start, getDecimalNumbers(solarNoonInSeconds, 0));
}
exports.getSolarNoonLSTDate = getSolarNoonLSTDate;
function getSunriseDate(params) {
    const { haSunrise, solarNoon } = getBaselineNumbersForSunriseAndSunset(params);
    const solarNoonInSeconds = solarNoon * 86400;
    const sunriseSeconds = solarNoonInSeconds - ((haSunrise * 4) / 1440) * 86400;
    const start = startOfDay_1.default(params.date);
    return addSeconds_1.default(start, getDecimalNumbers(sunriseSeconds, 0));
}
exports.getSunriseDate = getSunriseDate;
function getSunsetDate(params) {
    const { haSunrise, solarNoon } = getBaselineNumbersForSunriseAndSunset(params);
    const solarNoonInSeconds = solarNoon * 86400;
    const sunriseSeconds = solarNoonInSeconds + ((haSunrise * 4) / 1440) * 86400;
    const start = startOfDay_1.default(params.date);
...
exports.getSunsetDate = getSunsetDate;
