...
export function getJulianDate(date, tzOffset = date.getTimezoneOffset()) {
    const decimal = getDayFraction(date);
    const tzSubtractor = (-1 * tzOffset) / 1440;
    return (differenceInCalendarDays(date, new Date(1900, 0, 1, 12)) +
...
export function getSolarNoonLSTInSeconds(params) {
    const { solarNoon } = getBaselineNumbersForSunriseAndSunset(params);
    return solarNoon * 86400;
}
export function getSunriseInSeconds(params) {
...
export function getSunsetDate(params) {
    const { haSunrise, solarNoon } = getBaselineNumbersForSunriseAndSunset(params);
    const solarNoonInSeconds = solarNoon * 86400;
    const sunriseSeconds = solarNoonInSeconds + ((haSunrise * 4) / 1440) * 86400;
    const start = startOfDay(params.date);
...
export function getSolarElevationAngle({ date, latitude, longitude, tzOffset = date.getTimezoneOffset() }) {
    const { eq, sunDecl } = getBaselineNumbers(date, tzOffset);
    const trueSolarTime = (getDayFraction(date) * 1440 +
        eq +
        4 * longitude -
        60 * ((-1 * tzOffset) / 60)) %
        1440;
    ...
    const solarZenithAngle = degrees(acos(sin(radians(latitude)) * sin(radians(sunDecl)) +
